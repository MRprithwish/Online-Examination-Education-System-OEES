...
def student_dashboard(request):
    ...
    completed = sum(1 for course in my_courses if course.progress >= 100)
    avg = round(sum(course.progress for course in my_courses) / total) if total else 0
    ...
    for exam in exams:
        ...
        if now < start_time:
            ...
            exam.end_iso = end_time.isoformat()
...
def teacher_reply_message(request, ann_id):
  if request.method == 'POST':
    try:
      ...
      send_direct_message_email(reply)
...
@login_required(login_url='/exam_login/student/')
...
def exam_login(request, role):
    return render(request, "core/exam_login.html", {"role": role})
...
def student_results(request):
    ...
    return render(request, "core/student/student_results.html", {
        "results": results,
    })